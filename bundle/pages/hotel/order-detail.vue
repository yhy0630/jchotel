    ...
    <scroll-view scroll-y class="content">
      ...
      <view class="hotel-card">
        <view class="hotel-main">
          ...
          <view class="hotel-info">
            ...
            <view 
              ...
              v-if="order.type === 1 && ((order.request && order.request.city_name) || order.city_name)"
            >
              {{ (order.request && order.request.city_name) || order.city_name }}
              <text v-if="order.request && order.request.area">{{ order.request.area }}</text>
            </view>
            <!-- 定制房：展示附近参展物 / 地标 -->
            <view 
              class="hotel-landmark" 
              v-if="order.type === 1 && order.request && order.request.landmark"
            >
              附近参展物：{{ order.request.landmark }}
            </view>
            <view class="hotel-room" v-if="order.room_name">
              {{ order.room_name }}
            </view>
            <view class="hotel-checkin">
              入住当天18:00前可免费取消
            </view>
          </view>
        </view>
        ...
        <view class="total-price-inline">
          ...
          <text class="price-value">¥{{ order.amount_payable || 0 }}</text>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    getStatusText(status) {
      // 酒店订单（定制房 / 尊享房）统一显示“待入住”
      const map = { 0: '等待付款', 1: '待入住', 2: '已完成', 3: '已取消' }
