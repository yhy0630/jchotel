...
export function applyMember(data) {
    return request.post('member_apply/apply', data)
}

// 已开通的会员类型（商旅/渠道/企业/商务）
export function getMyMemberTypes() {
    return request.get('member_apply/myTypes')
}

// 入驻申请记录列表
export function getMemberApplyRecords() {
    return request.get('member_apply/myRecords')
}

// 入驻申请详情
export function getMemberApplyDetail(params) {
    return request.get('member_apply/detail', { params })
}

// 为入驻申请创建会员支付订单
export function createMemberOrder(data) {
    return request.post('member_apply/createOrder', data)
}

// 会员等级选项（按会员类型区分）
export function getMemberGradeOptions() {
    return request.get('member_apply/gradeOptions')
}
